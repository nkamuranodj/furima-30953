    ...
    describe '商品情報入力' do
      ...
      context '出品がうまくいかないとき' do
        ...
        it '送料の負担が選択されてないと登録できない' do
          @item.deliverypayer_id = 1
          @item.valid?
          expect(@item.errors.full_messages).to include 'Deliverypayer must be other than 1'
        end
        it '送料の負担が選択されてないと登録できない' do
          @item.deliverypayer_id = nil
          @item.valid?
          expect(@item.errors.full_messages).to include("Deliverypayer can't be blank")
        end
        ...
        it '発送までの日数が選択されてないと登録できない' do
          @item.shipday_id = 1
          @item.valid?
          expect(@item.errors.full_messages).to include 'Shipday must be other than 1'
        end
        it '発送までの日数が選択されてないと登録できない' do
          @item.shipday_id = nil
          @item.valid?
